...
def _build(
    ...
    theo = Distribution(
        np.array([theo_pos], dtype=float), np.full(len(theo_pos), 1.0 / len(theo_pos))
    )
...
def test_proportion_gradient_matches_fd_many_peaks():
    ...
    net = _build(
        list(np.linspace(-0.01, 0.01, 200)),
        max_distance=0.5,
        exp_trash=0.3,
        theo_trash=0.3,
        intensity_scale=1e6,
...
def test_truncation_path_unaffected():
    ...
    net = _build(
        [-1.0], max_distance=10.0, exp_trash=3.0, theo_trash=3.0, cost_scaling=False
    )
    grad, fd = _grad_and_fd(net)
